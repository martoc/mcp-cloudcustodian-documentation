...
from mcp_cloudcustodian_documentation.server import run_server
...
def test_run_server_defaults_to_stdio(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("mcp_cloudcustodian_documentation.server.mcp.run") as mock_run:
        run_server()
...
def test_run_server_http_defaults_host_and_port(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that MCP_TRANSPORT=http falls back to 0.0.0.0:8000 without host/port env vars."""
    monkeypatch.setenv("MCP_TRANSPORT", "http")
