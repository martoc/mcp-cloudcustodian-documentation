"""FastMCP server for Cloud Custodian documentation search and retrieval."""
...
DEFAULT_DB_PATH = Path(__file__).parent.parent.parent / "data" / "cloudcustodian_docs.db"

# Initialise FastMCP server
mcp = FastMCP(name="cloud-custodian-documentation")
...
def search_documentation(
    ...
    Args:
        query: Search terms to find in the documentation. Supports
               full-text search with stemming (e.g., "policy" matches
               "policies").
        section: Optional section to filter results. Common sections include:
                 'aws', 'azure', 'gcp', 'kubernetes', 'oci', 'quickstart',
                 'developer', 'tools', etc.
...
def read_documentation(path: str) -> str:
    """Read the full content of a specific Cloud Custodian documentation page.

    Args:
        path: The relative path to the documentation file (e.g.,
              'aws/examples/s3.rst' or 'quickstart/index.rst'). This path
              is returned in search results.

    Returns:
        The full RST content of the documentation page, or an error
